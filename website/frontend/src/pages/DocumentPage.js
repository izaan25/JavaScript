...
export default function DocumentPage() {
  ...
  const isCode = doc.fileType === 'javascript' || doc.fileType === 'js' || doc.fileType === 'code';
  const extension = isCode ? '.js' : '.md';

  return (
    <article className={`fade-up doc-container ${isCode ? 'code-mode' : 'reading-mode'}`}>
      {/* Breadcrumb */}
      <nav className="breadcrumb">
        ...
        <span className="sep">/</span>
        <span className="active-path">{doc.title}{extension}</span>
      </nav>

      {/* Header - Only for Non-Code/Documentation files */}
      {!isCode && (
